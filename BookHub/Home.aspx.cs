...
/*        private void BindCategory()
        {
            ...
            int maxCategoryDisplay = 4;
            for (int i = 0; i <= maxCategoryDisplay; i++)
            {
                var genreBooks = booksRepository.GetBooksByCategory(categories[i].CategoryId);
                String bookHtml = "";
                foreach(Book book in genreBooks)
                {
                    string a = "abc"+ book.BookId;
                    bookHtml += $@"
                        <a class='product-item swiper-slide' href='BookDetail.aspx?id={book.BookId}'>
                            <img src='{book.image}' alt=''>
                            <h4>{book.BookName}</h4>
                ...
                string html = $@"<button class=""genre-btn""  onClick=""showBookWithGenre({bookHtml})"">{categories[i].CategoryName}</button>";
        ...
        private void BindCategory()
        {
            ...
            int maxCategoryDisplay = Math.Min(categories.Count, 4);
            StringBuilder sb = new StringBuilder();
            StringBuilder bookHtmlBuilder = new StringBuilder();
            for (int i = 0; i < maxCategoryDisplay; i++)
            {
                int categoryId = categories[i].CategoryId;
                List<Book> genreBooks = booksRepository.GetBooksByCategory(categoryId);
                
                foreach (Book book in genreBooks)
                {
                    bookHtmlBuilder.Append($@"
                <div class='product-item swiper-slide' href='BookDetail.aspx?id={book.BookId}'>
                    <img src='{book.image}' alt=''>
                    <h4>{book.BookName}</h4>
                    <p>{book.Author}</p>
                    <div class='fav-price-container'>
                ...
                string genreHtml = $@"<button class=""genre-btn"">{categories[i].CategoryName}</button>";
                sb.Append(genreHtml);
            }
            category_container.InnerHtml = sb.ToString();
            genreBooks.InnerHtml= bookHtmlBuilder.ToString();
        }
